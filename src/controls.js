...
export class GalleryControls {
  ...
  update(dt) {
    // Kierunek ruchu względem yaw (płaski, niezależny od pitch)
    ...
    if (fwd !== 0 || strafe !== 0) {
      ...
      // Ograniczenie do wnętrza sali
      const margin = 0.6;
      this.player.x = Math.max(-ROOM.width / 2 + margin, Math.min(ROOM.width / 2 - margin, this.player.x));
      this.player.z = Math.max(-ROOM.depth / 2 + margin, Math.min(ROOM.depth / 2 - margin, this.player.z));
