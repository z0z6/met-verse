...
// (Odrobinę zmniejszone względem poprzedniej wartości 0.95.)
const ANDROID_MODEL_TARGET_SIZE = 0.85;
...
// Środek osi obrotu gogli (X=0 w jednostkach świata) domyślnie wypada na
// środku CAŁEGO ekranu — ten sam punkt co środek panelu wyboru trybu
// (który też jest wyśrodkowany na szerokość ekranu, patrz #intro
// w style.css). W praktyce jednak model, mimo geometrycznego centrowania
// (patrz komentarz przy Box3 niżej), wizualnie "cięży" w lewo na wąskich
// ekranach Androida — efekt pochylenia (applyTilt) w rzucie perspektywicznym
// przesuwa POZORNY środek bryły w bok, mimo że jej środek geometryczny
// zostaje w X=0. Dodatnia wartość przesuwa gogle w prawo, wyrównując ich
// wizualny środek ciężkości ze środkiem symetrii okna wyboru trybu.
// Sprawdź na realnym telefonie z Androidem i dostrój w razie potrzeby.
const ANDROID_MODEL_X_OFFSET = 0.35;
...
export function init(containerId = "canvas-container", options = {}) {
  ...
  if (IS_MOBILE && raiseOnMobile) {
    ...
    rig.position.x = IS_ANDROID ? ANDROID_MODEL_X_OFFSET : 0;
  }
